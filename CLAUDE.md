...
- Detalhes da 6.2/6.3 que valem lembrar:
  ...
  - **Bundle:** com as telas novas o pacote único foi a 618 KB. Agora **só `Login` e `Home` nascem no pacote principal** e todo o resto entra por `React.lazy` — carga inicial **581 KB** (gzip 175 KB), precache com 44 entradas e o `.woff2` dentro.
...
- **O projeto já roda contra Postgres real.** `npx supabase start` (com Docker Desktop aberto) aplica as 7 migrations e o seed; `npm run test:integracao` roda 50 testes contra esse banco, incluindo o isolamento entre ministérios que o planejamento chama de teste mais importante. `npm test` roda 79 testes puros e não precisa de Docker.
